...
?>
    ...
    <h1 class="invoice-title <?php echo $text_class ?>"><?php _trans('invoice') ?> <?php _htmlsc(date('Y') . '-' . $invoice->invoice_number) ?></h1>
    ...
    <table class="pdf-footer">
        <tr>
            <td>
                ...
                <?php } ?>
                <div>
                    <?php echo htmlsc(trim(($invoice->user_zip ?: '') . ' ' . ($invoice->user_city ?: ''))); ?>
                </div>
            </td>
            <td>
...
<sethtmlpagefooter name="spindlhofFooter" value="on" />
</body>
</html>